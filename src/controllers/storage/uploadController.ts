...
const postUploadFile = asyncHandler<AuthenticatedRequest>(async (req, res, next) => {
  ...
  const { successfulCount, failedCount, errors } = await userServiceInstance.uploadAndRegisterFiles(
    userId,
    publicFolderId,
    files,
  );
