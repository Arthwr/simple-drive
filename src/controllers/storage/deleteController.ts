...
const postDeleteFolder = asyncHandler<AuthenticatedRequest>(async (req, res, next) => {
  ...
  try {
    ...
    return next(new NotifyError(FlashMessages.STORAGE.FOLDER_DELETE_FAILED, 500));
  }
});

const postDeleteFile = asyncHandler<AuthenticatedRequest>(async (req, res, next) => {
  const userId = req.user.id;
  const publicFolderId = req.params.parentFolderId;
  const publicFileId = req.params.fileId;

  try {
    await userServiceInstance.deleteUserFile(userId, publicFolderId, publicFileId);
    ...
    return next(new NotifyError(FlashMessages.STORAGE.FILE_DELETE_FAILED, 500));
  }
});
